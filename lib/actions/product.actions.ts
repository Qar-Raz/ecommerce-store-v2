...
export async function getLatestProducts() {
  const query = sql`SELECT * FROM product ORDER BY "createdAt" DESC LIMIT 4;`
  const data = await db.execute(query)
  return data.rows
}

export async function getExpensiveProducts() {
  const query = sql`SELECT * FROM product ORDER BY "price" DESC LIMIT 4;`
  const data = await db.execute(query)
  return data.rows
}

export async function getHighlyRatedProducts() {
  const query = sql`SELECT * FROM product ORDER BY "rating" DESC LIMIT 4;`
  const data = await db.execute(query)
  return data.rows
}
...
export async function getProductBySlug(slug: string) {
  const query = sql`SELECT * FROM product WHERE slug = ${slug} LIMIT 1;`
